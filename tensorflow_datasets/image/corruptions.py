...
def clipped_zoom(img, zoom_factor):
  ...
  img = tfds.core.lazy_imports.scipy_ndimage.zoom(
...
def frosted_glass_blur(x, severity=1):
  ...
  c = [(0.7, 1, 2), (0.9, 2, 1), (1, 2, 3), (1.1, 3, 2), (1.5, 4,
                                                          2)][severity - 1]
  x = np.uint8(
      tfds.core.lazy_imports.skimage_filters.gaussian(
...
def brightness(x, severity=1):
  ...
  x = np.array(x) / 255.
  x = tfds.core.lazy_imports.skimage_color.rgb2hsv(x)
  x[:, :, 2] = np.clip(x[:, :, 2] + c, 0, 1)
  x = tfds.core.lazy_imports.skimage_color.hsv2rgb(x)
  x_clip = np.clip(x, 0, 1) * 255
  return around_and_astype(x_clip)
...
def elastic(x, severity=1):
  ...
  dx = (tfds.core.lazy_imports.skimage_filters.gaussian(
      np.random.uniform(-1, 1, size=shape[:2]),
      c[1],
      mode='reflect',
      truncate=3) * c[0]).astype(np.float32)
  dy = (tfds.core.lazy_imports.skimage_filters.gaussian(
      np.random.uniform(-1, 1, size=shape[:2]),
      c[1],
      mode='reflect',
      truncate=3) * c[0]).astype(np.float32)
  ...
  indices = np.reshape(y + dy, (-1, 1)), np.reshape(x + dx,
                                                    (-1, 1)), np.reshape(
                                                        z, (-1, 1))
  x_clip = np.clip(
      tfds.core.lazy_imports.scipy_ndimage.interpolation.map_coordinates(
